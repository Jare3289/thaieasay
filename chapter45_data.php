 ...
 *   ตัวอย่าง (1)-(N)  ข้อความจริงจากผลงานนักเรียน อย่างน้อย 1 คู่ต่อ 1 ตัวบ่งชี้ (ระบบคัดจากคลังที่นี่)
 ...
 *
 * ไม่มีเลขตัวอย่างตายตัวติดกับตัวบ่งชี้อีกต่อไป — ดู ch45_number_examples() ด้านล่าง
 */
function ch45_indicators() {
    ...
    foreach (ai_rubric() as $it) {
        ...
        $cache[$id] = array_merge($meta[$id], [
            ...
            // ไม่มีเลขตัวอย่างตายตัวอีกต่อไป — แต่ละหัวข้ออาจยกตัวอย่างได้ตั้งแต่ 1 คู่ขึ้นไปตามข้อมูลจริง
            // (ดู CH45_MAX_EXAMPLE_PAIRS) เลขตัวอย่างที่แสดงจริงคำนวณสด ๆ ตอนแสดงผล/พิมพ์
            // จากจำนวนคู่ที่วิเคราะห์ไว้แล้วของแต่ละหัวข้อ ไล่ตามลำดับหัวข้อ (ดู ch45_number_examples())
        ]);
    }
    return $cache;
}
...
/**
 * กำหนดเลขตัวอย่าง (1)-(N) ให้ทุกคู่ตัวอย่างของทุกตัวบ่งชี้ ไล่ตามลำดับตัวบ่งชี้ 1.1 → 4.3
 * (ลำดับเดียวกับที่ใช้แสดงผลทั้งเอกสาร) เพราะแต่ละหัวข้อยกตัวอย่างได้ไม่เท่ากันแล้ว
 * (ตั้งแต่ 0 คู่ อย่าง 4.3 ไปจนถึงหลายคู่ตามที่ CH45_MAX_EXAMPLE_PAIRS อนุญาต)
 * เลขจึงคำนวณสด ๆ จากผลวิเคราะห์ที่มีอยู่จริงตรงนี้ที่เดียว ไม่ใช่ค่าตายตัว
 * ใช้ร่วมกันทั้งหน้าจอ (เทียบเท่าฝั่ง JS: c45ComputeExampleNumbering) ร่างที่พิมพ์ และไฟล์ Google Doc
 *
 * คืนค่า [job_key => [[เลขที่1, เลขที่2], ...]]  หนึ่งคู่ต่อหนึ่งสมาชิกอาร์เรย์
 */
function ch45_number_examples(array $results) {
    $out = [];
    $n = 0;
    foreach (ch45_indicators() as $id => $ind) {
...
/* =========================================================================
 * ส่วนที่ 7  คลังตัวอย่างข้อความจริงจากผลงานนักเรียน (ตัวอย่าง (1)-(N))
 * ========================================================================= */
...
function ch45_readiness(array $ds, array $quant, array $defects, array $mech) {
    ...
    $add($items, 'essays', 'ตัวบทเรียงความจริง (ใช้ยกตัวอย่างในบทที่ 4)',
