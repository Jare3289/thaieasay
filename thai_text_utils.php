...
// ตรวจการเว้นวรรครอบ "ๆ" (ไม้ยมก) — ต้องเว้นวรรคทั้งก่อนและหลัง "ๆ" เสมอ (เช่น "เด็ก ๆ วิ่งเล่น")
// คืนค่าเป็นรายการคำที่ไม่ซ้ำ (คำที่ลงท้ายด้วย "ๆ" ติดกับคำหน้า = ไม่ได้เว้นวรรคหน้า, หรือ
// "ๆ" ที่ติดกับคำถัดไปทันที = ไม่ได้เว้นวรรคหลัง)
function find_maiyamok_spacing_errors(string $text, array $confirmedWords = [], int $limit = 30): array
{
    ...
    for ($i = 0; $i < $n; $i++) {
        ...
        $flagWord = null;
        if ($word !== 'ๆ') {
            // คำ+ๆ ติดกัน แปลว่าไม่มีช่องว่างคั่นจากคำหน้า — ผิด (ต้องเว้นวรรคก่อน ๆ ด้วย)
            $flagWord = $word;
        } else {
            // "ๆ" ถูกตัดคำแยกออกมาเป็นคำเดี่ยว แปลว่ามีช่องว่างคั่นจากคำหน้าแล้ว (ถูกต้องส่วนหน้า)
            // ตรวจต่อว่าเว้นวรรคก่อนคำถัดไปด้วยหรือไม่
