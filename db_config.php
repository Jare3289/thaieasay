...
// ---------------------------------------------------------------------------
// ตารางของระบบ "ให้ข้อเสนอแนะเรียงความอัตโนมัติ"
// ---------------------------------------------------------------------------
// 1) app_settings        : ค่าตั้งค่าทั่วไปของระบบ (ครูกรอกผ่านหน้าเว็บ) เช่นผู้ให้บริการโมเดลภาษา / โมเดล / API key
// 2) essay_ai_feedback   : ผลการตรวจของระบบต่อเรียงความ 1 ฉบับ (1 คน 1 รอบ = 1 แถว ตรวจใหม่ทับของเดิม)
// 3) ai_usage_log        : บันทึกการเรียกใช้ระบบไว้ให้ครูดูย้อนหลังว่าใช้ไปกี่ครั้ง (ไม่ได้ใช้จำกัดโควตาแล้ว)
//
// สำคัญ: คะแนนอัตโนมัติเก็บแยกจากตาราง evaluations ของงานวิจัย จึงไม่ปะปนกับคะแนนครู/เพื่อน/ตนเอง
try {
    ...
    if (!$tAi || $tAi->rowCount() === 0) {
        ...
        safe_ddl($pdo, "
            CREATE TABLE IF NOT EXISTS essay_ai_feedback (
                ...
                score_overrides LONGTEXT,      -- JSON map ของข้อที่คะแนนถูกปรับหลังระบบตรวจ (ครูปรับเอง/สั่งระบบตรวจข้อนั้นใหม่)
                teacher_scores  TEXT,          -- JSON map ของข้อที่ครูให้คะแนนเอง (ระบบตรวจแทนไม่ได้)
                ...
                essay_hash      CHAR(40)     DEFAULT NULL,  -- ลายนิ้วมือเนื้อหาเรียงความ ณ ตอนที่ระบบตรวจ
                ...
                draft_comment   TEXT,          -- ระบบสรุปว่าฉบับนี้ต่างจากฉบับตั้งต้นอย่างไร
...
// ข้อ 4.3 ความเรียบร้อย/ลายมือ ระบบตรวจจากไฟล์พิมพ์ไม่ได้ ครูจึงต้องกรอกคะแนนเองให้ครบเต็ม 60
try {
...
// เมื่อนักเรียนแก้ไขต้นฉบับหลังระบบตรวจไปแล้ว ระบบจะทำเครื่องหมายไว้ให้คุณครูสั่งตรวจใหม่
try {
...
// คุณครูปรับคะแนนข้อที่ไม่เห็นด้วยได้ หรือสั่งให้ระบบตรวจเฉพาะข้อนั้นใหม่ได้
...
// ตาราง "ภาพรวมการนำเสนอรายรอบงาน" — ระบบสรุปว่าทั้งชั้นนำเสนอภาระงานนั้นไปทางใด
...
// ---------------------------------------------------------------------------
// 1) ch45_analysis      : ผลวิเคราะห์ของระบบแยกตามหัวข้อ (job) หัวข้อละ 1 แถว ทำซ้ำจะทับของเดิม
