    ...
    <div class="card-body p-4">
      ...
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        ...
        <div class="d-flex gap-2 flex-wrap align-items-center">
          ...
          <a href="writing_feedback.php" class="btn btn-sm rounded-pill px-3 text-white"
             style="background:linear-gradient(135deg,#6d28d9,#0d7377);">
            <i class="bi bi-file-earmark-check me-1"></i>ระบบตรวจอัตโนมัติ
          </a>
        </div>
      </div>
...
<script>
  ...
  function buildPhaseCell(rec, phaseKey, graded) {
    ...
    // และปุ่มตรวจอัตโนมัติสำหรับสั่ง/ดูผลตรวจของระบบในหน้า writing_feedback.php
    ...
    return `<td class="${cls}">
      <div class="d-inline-flex gap-1">
        ...
        <button class="btn btn-sm btn-outline-primary rounded-circle p-0 d-inline-flex align-items-center justify-content-center"
          style="width:32px;height:32px;" title="ระบบตรวจอัตโนมัติ: ตรวจ/ดูข้อเสนอแนะของฉบับนี้ (${label})"
  ...
  // เปิดหน้าระบบตรวจอัตโนมัติโดยเลือกนักเรียนและรอบงานไว้ให้พร้อมตรวจ/ดูผลทันที
  function openAiFeedback(studentId, phase) {
    const params = new URLSearchParams();
    params.set('student_id', studentId);
    params.set('phase', phase);
    window.location.href = 'writing_feedback.php?' + params.toString();
  }
